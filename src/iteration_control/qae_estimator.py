...
def qae_full(
    ...
    # Phase phi = phase_int / 2^n_ancilla
    # Eigenvalues of Q are e^{i(2*theta + pi)} due to the -1 factor in our diffusion
    # So phase = (2*theta + pi) / 2*pi = theta/pi + 0.5
    best = max(counts, key=counts.get)
    phase_int = int(best, 2)
    phi = phase_int / (2 ** n_ancilla)
    
    # theta = pi * |phi - 0.5|
    theta = np.pi * abs(phi - 0.5)
